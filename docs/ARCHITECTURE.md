...
Important failures must be diagnosable without logging secrets or unnecessary sensitive data. Implemented durable mutations record scoped audit events and correlation/request context. Work-dependency mutations additionally preserve durable active/removed history and idempotent request results; durable AI reviews retain bounded target/context, provider/model provenance, completion/failure state, and owner disposition when recorded. Later observability work must extend this baseline to broader links, later requirement lifecycle, later artifact versions/import, broader AI-review/stale-review behavior, evidence freshness and verification decisions, dependency/change-impact, and release workflows.
...
Dependency create/remove operations are transactional and idempotent. Injected audit failure rolls back the graph mutation and request result. Active and removed history survives re-login. Dependency records do not automatically block, resume, complete, reopen, verify, validate, release, satisfy readiness, or propagate impact. Broader durable links and dependency-impact semantics remain future scope.
