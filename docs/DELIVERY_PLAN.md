...
17. **Product acceptance-criterion withdrawal** — Validated at application/test head `8e6c267ae737689691f313999c0f792786954a71`, CI run 383. See [slice](INCREMENT_2_CRITERION_WITHDRAWAL.md) and [validation](validation/increment-2-criterion-withdrawal.md). The owner can withdraw one active condition from an approved product requirement without deleting criterion or evidence history; at least one active condition must remain, technical criteria are excluded from the owner-only path, and withdrawn conditions cannot receive new evidence. Withdrawal does not establish evidence freshness, verification, failure, validation, or release state.
18. **Development AI provider selection and traffic observability** — Validated at application/test head `8ff850afb99eb6a9e73635b700ae2543086826eb`, CI run 399. See [ADR-0007](adr/0007-ai-provider-selection-and-development-observability.md), [slice](INCREMENT_2_AI_DEVELOPMENT_CONSOLE.md), and [validation](validation/increment-2-ai-development-console.md). LM Studio is the default development provider; discovered LM Studio/Ollama models are selectable for local testing and normal local AI-review routing; an explicitly configured OpenAI connection can be tested without exposing its key; sanitized transient traffic/metrics are visible in the development console; and durable project review remains local-only with no silent cloud use or local fallback when OpenAI is selected.

### Active remaining Increment 2 scope

The active single-user scope now prioritizes:

1. remaining acceptance-criterion lifecycle beyond validated addition and withdrawal, including criterion text revision/change semantics, reactivation, replacement/supersession/deprecation where approved, and any future technical multi-criterion design;
...
The validated development AI console is temporary troubleshooting instrumentation. Durable public-provider AI review remains future scope and requires a separate approved provenance/schema slice; a successful public-provider connection test does not authorize durable cloud project review.
