...
import type { LocalAiStatus } from "@/lib/ai/local-ai";
    ...
    <details className="work-action ai-review-disposition">
      ...
      <form action={action} className="durable-form" aria-label={`AI review disposition: ${item.title}`}>
        ...
        <label htmlFor={`ai-disposition-${review.id}`}>
          ...
          <select id={`ai-disposition-${review.id}`} name="disposition" value={disposition} onChange={(event) => setDisposition(event.target.value as AiReviewDisposition)}>
        ...
        <label htmlFor={`ai-disposition-note-${review.id}`}>
          Why?
          <textarea id={`ai-disposition-note-${review.id}`} name="note" rows={3} maxLength={2000} required value={note} onChange={(event) => setNote(event.target.value)} />
        </label>
        <label className="owner-confirm" htmlFor={`ai-disposition-confirm-${review.id}`}>
          <input id={`ai-disposition-confirm-${review.id}`} name="confirm" type="checkbox" required checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />
        ...
        <button className="button secondary" type="submit" disabled={pending}>{pending ? "Saving disposition…" : "Save disposition"}</button>
      </form>
    </details>
  );
}
...
function ReviewCard({ item, review }: { item: WorkItemRecord; review: AiReviewRecord }) {
  return (
    <article className="ai-review-card" aria-label={`AI review ${review.status}`}>
      ...
      <p className="form-help">Context: saved work-item record only · Target revision {review.target_revision}</p>
      {review.status === "Completed" ? (
        <>
          <div className="ai-review-result">{review.advisory_result}</div>
...
export function AiWorkReview({ item, requestId }: { item: WorkItemRecord; requestId: string }) {
  ...
  useEffect(() => {
    ...
    void fetch("/api/local-ai/status", { cache: "no-store" })
      .then(async (response) => response.ok ? response.json() as Promise<LocalAiStatus> : null)
      .then((status) => { if (active) setLocalAi(status); })
      .catch(() => { if (active) setLocalAi(null); })
      .finally(() => { if (active) setStatusChecked(true); });
    return () => { active = false; };
  }, []);

  if (item.status !== "Implemented") return null;
  const localAiAvailable = Boolean(localAi?.provider && localAi?.model);
  ...
  return (
    <section className="ai-review-section" aria-labelledby={`ai-review-title-${item.id}`}>
      ...
      {!statusChecked ? <p className="form-help">Checking local AI…</p> : localAiAvailable ? (
        <details className="work-action ai-review-request">
          ...
          <form action={action} className="durable-form" aria-label={`Ask local AI to review: ${item.title}`}>
            ...
            <label htmlFor={`ai-review-purpose-${item.id}`}>
              ...
              <textarea id={`ai-review-purpose-${item.id}`} name="purpose" rows={3} maxLength={2000} required value={purpose} onChange={(event) => setPurpose(event.target.value)} />
            </label>
            <label className="owner-confirm" htmlFor={`ai-review-confirm-${item.id}`}>
              <input id={`ai-review-confirm-${item.id}`} name="confirm" type="checkbox" required checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />
              <span>I understand that only this saved work record is sent to local AI and the result is advisory, not verification.</span>
            </label>
            {state.error ? <p role="alert" className="form-error">{state.error}</p> : null}
            <button className="button secondary" type="submit" disabled={pending}>{pending ? "Reviewing with local AI…" : "Run AI review"}</button>
          </form>
        </details>
      ...
      {item.ai_reviews.length ? (
        <div className="ai-review-list">{item.ai_reviews.map((review) => <ReviewCard key={review.id} item={item} review={review} />)}</div>
