...
export class ArtifactStore {
  ...
  async list(workspaceId: string): Promise<ArtifactRecord[]> {
    const { data, error } = await this.rpc("list_artifacts", { p_workspace: workspaceId });
    if (error) {
  ...
  async create(input: CreateArtifactInput): Promise<string> {
    const { data, error } = await this.rpc("create_proposed_artifact", {
