-- Durable owner-authored decisions extend the existing private workspace boundary.
...
create function wayfound.create_decision(
 p_workspace uuid,
 p_title text,
 p_decision text,
 p_rationale text,
...
begin
 ...
 insert into wayfound.decisions(workspace_id,release_id,stage_number,title,decision_statement,rationale,status,actor_id)
 values(p_workspace,release,stage,btrim(p_title),btrim(p_decision),btrim(p_rationale),'Proposed',actor)
 returning id into decision_id;
 insert into wayfound.audit_events(workspace_id,actor_id,operation,entity_id,correlation_id)
 values(p_workspace,actor,'decision.created',decision_id,p_request);
...
revoke all on function wayfound.list_decisions(uuid), wayfound.create_decision(uuid,text,text,text,uuid) from public, anon, authenticated;
grant execute on function wayfound.list_decisions(uuid), wayfound.create_decision(uuid,text,text,text,uuid) to authenticated;
...
create function public.create_decision(p_workspace uuid,p_title text,p_decision text,p_rationale text,p_request uuid) returns uuid language sql security invoker set search_path = '' as $$select wayfound.create_decision(p_workspace,p_title,p_decision,p_rationale,p_request)$$;
revoke all on function public.list_decisions(uuid), public.create_decision(uuid,text,text,text,uuid) from public, anon;
grant execute on function public.list_decisions(uuid), public.create_decision(uuid,text,text,text,uuid) to authenticated;
